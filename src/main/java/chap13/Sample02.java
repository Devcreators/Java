...
public class Sample02 {
	...
	public static void main(String[] args) {
		// 추상 클래스이므로 생성자를 사용하여 인스턴스를 생성할 수 없어서
		// static 메서드를 호출하여 생성
		...
		System.out.println();
		System.out.println("=== Date ===");
		System.out.printf("%s %s %02d %02d:%02d:%02d KTS %d\n"
				, getWeekName(date.getDay() + 1)
				, getMontName(date.getMonth())
				, date.getDate()
				, date.getHours()
				, date.getMinutes()
				, date.getSeconds()
		...
		System.out.printf("%s %s %02d %02d:%02d:%02d KTS %d\n"
				...
				, getMontName(cal.get(Calendar.MONTH))
	...
	public static String getMontName(int MontNumber) {
		var returnVal = switch (MontNumber) {
		...
		default -> throw new IllegalArgumentException("Unexpected value : " + MontNumber);
		};
		return returnVal;
	}
